		...
		private void LoadValuesControls(int _option, Cne _cneData)
		{
			...
			else
			{
				if (!_cneData.Descripcion.Contains("El usuario no se encuentra"))
				{
					...
					this.FullName = _cneData.Nombre.Trim();
					this.State = _cneData.Estado.Trim();
					this.Municipality = _cneData.Municipio.Trim();
					this.Parish = _cneData.Parroquia.Trim();
					this.Center = _cneData.Centro.Trim();
					this.Address = _cneData.Direccion.Trim();
				}
				else
				{
